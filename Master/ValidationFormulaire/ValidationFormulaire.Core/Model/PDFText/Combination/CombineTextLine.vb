            ...
            If Math.Abs(row.Key.Y - current_y) > MAX_DISTANCE_BETWEEN_CHARACTERS_FOR_Y Then
                If words.Count > 0 Then
                    ...
                    For Each word As KeyValuePair(Of Point, String) In words
                        If previous_word_x <> 0 AndAlso
                                Math.Abs(word.Key.Y - previous_word_x) > MAX_DISTANCE_BETWEEN_CHARACTERS_FOR_X Then
                            word_string += " "
                        End If
                ...
                If Not words.ContainsKey(row.Key.X) Then
                    words.Add(New Point(row.Key.X, row.Value.right_side), row.Value.text)
                Else
                    words(New Point(row.Key.X, row.Value.right_side)) += row.Value.text
                End If
            Else
                If Not words.ContainsKey(row.Key.X) Then
                    words.Add(New Point(row.Key.X, row.Value.right_side), row.Value.text)
