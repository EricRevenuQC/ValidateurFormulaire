...
<TestClass()> Public Class AuthorizationTests
    ...
    <TestMethod()> Public Sub VerifyThatDataDictionaryIsProperlyFilled()
        ...
        bar_code_reader.Setup(Function(f) f.GetBarCodeId()).Returns("13RS")
        extractor.Setup(Sub(f) f.PDFToText(file_mock.Object, True))
        extractor.Setup(Function(f) f.GetWords()).Returns(pdf_words)
    ...
    <TestMethod()> Public Sub VerifyThatAllAuthorizationLogicFunctionsAreCalledInCorrectOrder()
        ...
        bar_code_reader.Setup(Function(f) f.GetBarCodeText()).Returns("13RSFS99999992017D")
        bar_code_reader.Setup(Function(f) f.GetBarCodeId()).Returns("13RS")
        extractor.InSequence(sequence).Setup(Sub(f) f.PDFToText(file_mock.Object, True))
        extractor.InSequence(sequence).Setup(Function(f) f.GetWords()).Returns(pdf_words)
        ...
        extractor.Verify(Sub(f) f.PDFToText(file_mock.Object, True), Times.Exactly(1))
